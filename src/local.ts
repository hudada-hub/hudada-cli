...
import fs,{ createReadStream, createWriteStream, mkdirSync, existsSync, statSync, readdirSync, readFileSync, rm, rmSync } from 'fs';
import path, { join, extname, dirname, relative, isAbsolute,basename } from 'path';
...
async function handleStaticFile(req: IncomingMessage, res: ServerResponse, path: string) {
    try {
        // 移除 URL 中的 /static 前缀
        const relativePath = decodeURIComponent(path.replace(/^\/static/, ''));
        // 构建完整的文件路径
        const filePath = join(__dirname,'template', relativePath);

        console.log(filePath,'handleStaticFile');
...
async function handlePreviewFile(req: IncomingMessage, res: ServerResponse, path: string) {
    try {
        // 移除 URL 中的 /static 前缀
        const relativePath = decodeURIComponent(path.replace(/^\/preview/, ''));
        // 构建完整的文件路径
        ...
        if (filePath.endsWith('.md')) {
            try {
                let mdContent = await fs.readFileSync(filePath, 'utf-8');
                console.log(mdContent,'mdContent');
  // 清理 Markdown 内容
  mdContent = cleanMarkdownContent(mdContent);
  console.log('清理后的 Markdown 内容:', mdContent);
  const marked = new Marked(
    ...
    <body>
        ${htmlContent}
                ...
                res.writeHead(404, { 'Content-Type': 'text/plain;chatset=utf-8' });
                res.end('Markdown 文件未找到');
            }
            return;
        }
...
export async function startLocalServer(port = 667) {
    ...
    const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
        try {
            ...
            console.log('请求路径:', decodeURIComponent(path));
               // 读取当前工作目录
            ...
            if (path.startsWith('/static/')) {
             ...
             if (path.startsWith('/preview/')) {
                await handlePreviewFile(req, res, path);
                return;
            }
            ...
            if (path === '/') {
                res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
                // const files = await fs.readdirSync(cwd);
               ...
               // 处理 Markdown 文件渲染
        let filePath =join(cwd, path.slice(1));
    ...
    // 启动服务器
    server.listen(port, () => {
        let baseDir=  path.normalize(process.cwd());
...
async function handleFileDelete(req: IncomingMessage, res: ServerResponse, query: any) {
    try {
        ...
        // 检查路径是否在 uploads 目录内
        const relativePath = relative(join(process.cwd()), fullPath);
...
function getHtmlContent() {
    ...
    if (existsSync(currentDir)) {
        fileStructure = getFolderStructure(currentDir);
    }
    try {
        // 读取 HTML 模板
